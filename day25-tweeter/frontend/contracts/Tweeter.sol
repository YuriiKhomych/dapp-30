...
abstract contract Resolver {
    function addr(bytes32 node) view external virtual returns (address);
}

abstract contract ENS {
    function resolver(bytes32 node) view external virtual returns (Resolver);
}
...
contract Tweeter {
    ...
    function _sendMessage(
        ...
        internal
        canOperate(_from) {
        uint256 conversationId = uint256(uint160(_from)) + uint256(uint160(_to));
