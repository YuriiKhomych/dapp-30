...
contract('Lottery', (accounts) => {
  ...
  it('Should bet', async () => {
    ...
    await lottery.createBet(3, web3.utils.toWei('1'));
    ...
    const result = players.some((_player, i) => {
      ...
      const expected = web3.utils.toBN(web3.utils.toWei('1.9'));
      return balancesAfter[i].sub(balancesBefore[i]).add(gasUsed).gte(expected);
    });
    assert(result === true);
  });
